...
export class MongoService extends CommandRunner {
  ...
  async seed(options?: MongoCommandOptions): Promise<void> {
    await this.mongoProvider.connect();
    console.log('Seeding mongo...');

    for (const collectionName of Object.keys(seeds)) {
      if (this.cond(collectionName, options)) {
        await this.mongoProvider
          .db()
          .collection(collectionName)
          .insertMany(seeds[collectionName]);

        console.log(
          '\x1b[32m%s\x1b[0m',
          `Inserted ${seeds[collectionName].length} documents into the ${collectionName} collection`,
        );
      }
    }
